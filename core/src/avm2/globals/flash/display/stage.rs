...
pub fn set_quality<'gc>(
    ...
    if let Ok(quality) = args.get_string(activation, 0).parse::<ruffle_render::quality::StageQuality>() {
        ...
        // throws away MSAA and bitmap downsampling for no reason. Honour
        // upgrades; ignore drops.
        let current = activation.context.stage.quality();
        let new_quality = if quality.sample_count() < current.sample_count() {
            current
        } else {
            quality
        };
        activation
            .context
            .stage
            .set_quality(activation.context, new_quality);
    }
    Ok(Value::Undefined)
}
