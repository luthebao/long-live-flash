...
/// Returns `true` when this object has a `scale9Grid` set and its local matrix
/// applies a non-trivial axis-aligned scale that should be remapped piecewise.
fn needs_scaling_grid(this: DisplayObject<'_>) -> bool {
    let grid = this.scaling_grid();
    if !grid.is_valid() || grid.width() <= Twips::ZERO || grid.height() <= Twips::ZERO {
        return false;
    }
    let m = this.base().matrix();
    // 9-slice with rotation/skew is rare and unsupported here; bail to uniform scaling.
    if m.b.abs() > 1e-3 || m.c.abs() > 1e-3 {
        return false;
    }
    (m.a - 1.0).abs() > 1e-3 || (m.d - 1.0).abs() > 1e-3
}

/// Render `this`'s children using 9-slice scaling defined by `scale9Grid`.
///
/// Called only when [`needs_scaling_grid`] returned `true`. The transform
/// stack on entry has `ancestors_world * this.matrix()` on top; we replace
/// the local scale with a per-region piecewise-linear map, masking each pass
/// to its destination rectangle so that overdraw from shapes that span
/// multiple slices is clipped to the slice they belong to.
fn render_with_scaling_grid<'gc>(this: DisplayObject<'gc>, context: &mut RenderContext<'_, 'gc>) {
    let grid = this.scaling_grid();
    // `self_bounds` for a MovieClip only covers its own draw-surface; the
    // scale9 content is typically in the clip's children. `bounds(Engine)`
    // recurses through children at identity transform to give the natural,
    // unscaled local-space bounds.
    ...
    let world_transform = context.transform_stack.transform();
    let local_matrix = this.base().matrix();
    let local_color_transform = this.base().color_transform();
    let local_perspective_projection = this.base().perspective_projection();
    let Some(inv_local) = local_matrix.inverse() else {
        this.render_self(context);
        return;
    };
    let ancestors_matrix = world_transform.matrix * inv_local;

    let sx = local_matrix.a;
    let sy = local_matrix.d;

    // Clamp grid to bounds and resolve slice positions in local twips.
    let bx_min = bounds.x_min.get() as f32;
    let by_min = bounds.y_min.get() as f32;
    let bx_max = bounds.x_max.get() as f32;
    let by_max = bounds.y_max.get() as f32;
    let gx_min = (grid.x_min.get() as f32).clamp(bx_min, bx_max);
    let gy_min = (grid.y_min.get() as f32).clamp(by_min, by_max);
    let gx_max = (grid.x_max.get() as f32).clamp(gx_min, bx_max);
    let gy_max = (grid.y_max.get() as f32).clamp(gy_min, by_max);

    // Source x/y partition boundaries (local-twip coords).
    let bx = [bx_min, gx_min, gx_max, bx_max];
    let by = [by_min, gy_min, gy_max, by_max];
    let sw = [bx[1] - bx[0], bx[2] - bx[1], bx[3] - bx[2]];
    let sh = [by[1] - by[0], by[2] - by[1], by[3] - by[2]];

    // Destination origin in parent-local coords = local_matrix * (bx_min, by_min).
    let dest_origin_x = sx * bx_min + local_matrix.tx.get() as f32;
    let dest_origin_y = sy * by_min + local_matrix.ty.get() as f32;

    // Dest slice widths (corners keep their natural source size; the
    // center absorbs whatever scale leaves over). Clamp center to 0 when
    // the corners alone are larger than the target.
    let total_w = sx * (sw[0] + sw[1] + sw[2]);
    let total_h = sy * (sh[0] + sh[1] + sh[2]);
    let dw = [sw[0], (total_w - sw[0] - sw[2]).max(0.0), sw[2]];
    let dh = [sh[0], (total_h - sh[0] - sh[2]).max(0.0), sh[2]];

    // Replace the local matrix on the stack with per-region matrices.
    context.transform_stack.pop();

    let mut dy_start = dest_origin_y;
    for j in 0..3 {
        let mut dx_start = dest_origin_x;
        for i in 0..3 {
            // Skip empty slices.
            if sw[i] <= 0.0 || sh[j] <= 0.0 || dw[i] <= 0.0 || dh[j] <= 0.0 {
                dx_start += dw[i];
                continue;
            }
            ...
            // Per-region local matrix:
            //   parent.x = s_a * (local.x - bx[i]) + dx_start
            //   parent.y = s_d * (local.y - by[j]) + dy_start
            let region_local = Matrix {
                a: s_a,
                b: 0.0,
                c: 0.0,
                d: s_d,
                tx: Twips::new((dx_start - s_a * bx[i]).round() as i32),
                ty: Twips::new((dy_start - s_d * by[j]).round() as i32),
            };

            context.transform_stack.push(&Transform {
                matrix: region_local,
                color_transform: local_color_transform,
                perspective_projection: local_perspective_projection,
            });

            // Mask the rendering of this region to its dest rect (parent-local
            // → world by composing with the ancestor world matrix). Without a
            // mask, shapes that span multiple slices would draw 9 stretched
            // copies on top of each other.
            let dest_rect = Rectangle {
                x_min: Twips::new(dx_start.round() as i32),
                y_min: Twips::new(dy_start.round() as i32),
                x_max: Twips::new((dx_start + dw[i]).round() as i32),
                y_max: Twips::new((dy_start + dh[j]).round() as i32),
            };
            let mask_matrix =
                ancestors_matrix * Matrix::create_box_from_rectangle(&dest_rect);
    ...
    // Restore the original local transform so render_base's cleanup balances.
    context.transform_stack.push(&Transform {
        matrix: local_matrix,
        color_transform: local_color_transform,
        perspective_projection: local_perspective_projection,
