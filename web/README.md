...
We compile Llflash down to a Wasm ([WebAssembly](https://webassembly.org/)) binary, which will be loaded
...
By default we will detect and replace any embedded Flash content on websites with the Llflash player
(we call this "polyfilling"), but this can be configured by the website. This means that Llflash is an
"out of the box" solution for getting Flash to work again; include Llflash and it should just work™.
...
This can be installed with `cargo install wasm-bindgen-cli --version 0.2.120`. Be sure to install this specific version of `wasm-bindgen-cli` to match the version used by Llflash.
...
You will need a BrowserStack account (Maintainers may contact @Dinnerbone on Discord for an invite to the Llflash team),
...
-   [packages/selfhosted](packages/selfhosted) is a node package intended for consumption by websites to include Llflash on their site.
-   [packages/extension](packages/extension) is a node package that turns Llflash into a browser extension.
