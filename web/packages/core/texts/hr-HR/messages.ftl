message-cant-embed =
    Llflash nije uspio pokrenuti Flash ugrađen na ovoj stranici.
...
llflash-desktop = Aplikacija za stolna računala
llflash-wiki = Pogledajte Llflash Wiki
enable-hardware-acceleration = Izgleda da je hardversko ubrzanje onemogućeno. Iako Llflash možda radi, mogao bi biti vrlo spor. Kako omogućiti hardversko ubrzanje možete saznati slijedeći donju poveznicu:
...
error-file-protocol =
    Čini se da koristite Llflash na protokolu "file:".
...
error-javascript-config =
    Llflash je naišao na veliki problem zbog netočne konfiguracije JavaScripta.
    Ako ste administrator poslužitelja, pozivamo vas da provjerite detalje pogreške kako biste saznali koji je parametar uzrok problema. Također možete konzultirati Llflash wiki za pomoć.
error-wasm-not-found =
    ...
    Ako se problem nastavi, možda ćete morati upotrijebiti postavku "publicPath": za pomoć se obratite Llflash wikiju.
error-wasm-mime-type =
    ...
    Ako ste administrator poslužitelja, obratite se Llflash wiki stranici za pomoć.
error-invalid-swf =
...
error-swf-fetch =
    ...
    Najvjerojatniji razlog je taj što datoteka više ne postoji, pa Llflash nema što učitati.
    Pokušajte se obratiti administratoru web-mjesta za pomoć.
error-swf-cors =
    Llflash nije uspio učitati Flash SWF datoteku.
    Pristup dohvaćanju vjerojatno je blokiran pravilom CORS.
    Ako ste administrator poslužitelja, za pomoć se obratite Llflash wikiju.
error-wasm-cors =
    Llflash nije uspio učitati potrebnu komponentu datoteke ".wasm".
...
error-csp-conflict =
    ...
    Ako ste administrator poslužitelja, za pomoć se obratite Llflash wikiju.
error-unknown =
    Llflash je naišao na ozbiljan problem prilikom pokušaja prikaza ovog Flash sadržaja.
    { $outdated ->
