import type { RuffleHandle, ZipWriter } from "../../../dist/llflash_web";
import {
    AutoPlay,
...
/**
 * An item to show in Llflash's custom context menu.
 */
...
export class InnerPlayer {
    ...
    /**
     * Determine if this element is the fallback content of another Llflash
     * player.
     *
     * This heuristic assumes Llflash objects will never use their fallback
    ...
    /**
     * Ensure a fresh Llflash instance is ready on this player before continuing.
     *
     * @throws Any exceptions generated by loading Llflash Core will be logged
     * and passed on.
     *
     * @private
     */
    private async ensureFreshInstance(): Promise<void> {
        ...
        console.log(
            "%c" +
                "New Llflash instance created (Version: " +
     ...
     *
     * @param bytesLoaded The size of the Llflash WebAssembly file downloaded so far.
     * @param bytesTotal The total size of the Llflash WebAssembly file.
     */
    ...
    /**
     * Destroys the currently running instance of Llflash.
     */
    destroy(): void {
        if (this.instance) {
            this.stopBackgroundTick();
            ...
            this.metadata = null;
            this._readyState = ReadyState.HaveNothing;
            console.log("Llflash instance destroyed.");
        }
    }
    ...
    async load(
        ...
        if (!this.element.isConnected || this.isUnusedFallbackObject()) {
            console.warn(
                "Ignoring attempt to play a disconnected or suspended Llflash element",
            );
            return;
        }

        if (isFallbackElement(this.element)) {
            // Silently fail on attempt to play a Llflash element inside a specific node.
            return;
        }
        ...
        try {
            ...
            // Pre-emptively set background color of container while Llflash/SWF loads.
            if (
    ...
    private fullScreenChange(): void {
        ...
        if (
            ...
            // when Llflash loads the fullScreenAspectRatio should be applied by `Stage.setAspectRatio`,
    ...
    protected openVirtualKeyboard(): void {
        ...
        // That is why we are checking whether Llflash already has focus:
     ...
     * of recovery.
     *
     * Llflash will attempt to isolate all damage to this specific player instance, but no guarantees can be made if there
     * was a core issue which triggered the panic. If Llflash is unable to isolate the cause to a specific player, then
     * all players will panic and Llflash will become "poisoned" - no more players will run on this page until it is
     * reloaded fresh.
     *
...
/**
 * The volume controls of the Llflash web GUI.
 */
