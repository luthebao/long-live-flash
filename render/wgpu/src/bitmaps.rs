...
impl BitmapSamplers {
    ...
    pub fn get_sampler(&self, is_repeating: bool, _is_smoothed: bool) -> &wgpu::Sampler {
        // Force nearest sampling for all bitmaps, ignoring the SWF's
        // per-bitmap `smoothing` flag.
        //
        // Why: the small SWFs we target (e.g. 500×375 design res) get played
        // in much larger windows. With smoothing=true (the SWF default) and a
        // non-integer scale, bilinear visibly blurs every bitmap UI element —
        // panels, button frames, the rounded teal "boxes". Nearest gives
        // pixel-perfect upscale instead. Pixel-art aliasing on character art
        // is the acceptable cost; crisp UI is what the user asked for.
        match is_repeating {
            true => &self.repeat_nearest,
            false => &self.clamp_nearest,
        }
    }
}
