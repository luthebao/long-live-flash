...
impl FilePicker {
    ...
    pub async fn pick_ruffle_file(&self, dir: Option<PathBuf>) -> Option<PathBuf> {
        ...
        let picked = if let Some(result) = self.show_dialog(dialog, |d| d.pick_file()) {
            result.await.map(|h| h.into())
        } else {
            None
        };
        self.notify_dialog_dismissed();
        picked
    }
    ...
    pub async fn pick_ruffle_directory(&self, dir: Option<PathBuf>) -> Option<PathBuf> {
        ...
        let picked: Option<PathBuf> = match self.show_dialog(dialog, |d| d.pick_folder()) {
            Some(fut) => fut.await.map(|h| h.into()),
            None => None,
        };
        self.notify_dialog_dismissed();
        picked
    ...
    /// Notify the event loop that a native file dialog has closed.
    ///
    /// On macOS, NSOpenPanel can detach the parent window's IME hook even after
    /// focus returns; the loop re-applies the cached `ime_allowed` state.
    /// No-op on other platforms.
    fn notify_dialog_dismissed(&self) {
        #[cfg(target_os = "macos")]
        {
            let _ = self.data.event_loop.send_event(RuffleEvent::ReArmIme);
        }
    }
