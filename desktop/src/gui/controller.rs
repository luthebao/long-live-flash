...
pub struct GuiController {
    ...
    /// Last value passed to `set_ime_allowed`, replayed by `reapply_ime_allowed`
    /// when something detached the underlying IME hook (e.g. NSOpenPanel on macOS).
...
impl GuiController {
    ...
    /// Re-push the cached `ime_allowed` flag to the window. Use after something
    /// outside winit (e.g. a macOS NSOpenPanel sheet) may have detached the IME
    /// hook on the first responder.
    pub fn reapply_ime_allowed(&self) {
        self.window.set_ime_allowed(self.ime_allowed.get());
    }
